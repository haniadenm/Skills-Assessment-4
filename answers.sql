...
SELECT name, SUM(num_cupcakes) FROM cupcakes LEFT JOIN orders ON orders.cupcake_id = cupcakes.id GROUP BY cupcakes.name ORDER BY cupcakes.name;
...
SELECT email FROM customers LEFT JOIN orders ON orders.customer_id = customers.id WHERE num_cupcakes >= 100 ORDER BY email DESC;
...
SELECT fname, lname, email from customers LEFT JOIN orders ON orders.customer_id = customers.id LEFT JOIN cupcakes on cupcakes.id = cupcake_id WHERE name = 'funfetti' AND processed = 't' GROUP BY fname, lname, email ORDER BY customers.email;